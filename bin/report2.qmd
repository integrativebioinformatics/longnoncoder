...
order_chromosomes <- function(x) {
  ...
  factor(x, levels = c(numeric_sorted, other_sorted))
}
```
...
create_plot <- function(data, x, fill, title) {
  ggplot(data, aes(x = {{x}}, y = count, fill = {{fill}})) +
    ...
    geom_point(position = position_dodge(width = 0.9), size = 2.4, shape = 21, colour = "grey20") +
...
> This image might render differently according to data. Access `assets/report_template.qmd`, and adjust the `fig-height` and `fig-width` parameters to better fit the plot if needed. The code chunk generating this plot is `plot-gb-across-chr`.
...
p9 <- ggplot(chr_gb, aes(x = gene_biotype, y = count)) +
  ...
  geom_point(aes(fill = gene_biotype), size = 2.2, shape = 21, colour = "grey20") +
...
p16 <- ggplot(count_momu, aes(x = transcript_biotype, y = count, fill = transcript_biotype)) +
  geom_linerange(aes(ymin = 1, ymax = count), colour = "grey65", linewidth = 0.5) +
  geom_point(size = 2.2, shape = 21, colour = "grey20") +
...
p20 <- ggplot(class_counts, aes(x = classification, y = count, fill = classification)) +
  geom_linerange(aes(ymin = 1, ymax = count), position = position_dodge(width = 0.9),
                 colour = "grey65", linewidth = 0.5) +
  geom_point(position = position_dodge(width = 0.9), size = 2.2, shape = 21, colour = "grey20") +
...
> This image might render differently according to data. Access `assets/report_template.qmd`, and adjust the `fig-height` and `fig-width` parameters to better fit the plot if needed. The code chunk generating this plot is `plot-novel-across-chr`.
...
p21 <- ggplot(chr_tb_novel, aes(x = biotype, y = count)) +
  geom_linerange(aes(ymin = 1, ymax = count), colour = "grey65", linewidth = 0.5) +
  geom_point(aes(fill = biotype), size = 2.2, shape = 21, colour = "grey20") +
