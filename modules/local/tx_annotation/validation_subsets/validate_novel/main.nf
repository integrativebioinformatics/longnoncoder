process BAMBU_VALIDATE {
    ...
    input:
    tuple val(meta1), path(metadata_csv)
...
SCRIPT_DIR="\$(pwd)"
METADATA_FILE="${SCRIPT_DIR}/novel_pc_lnc_RNAs_metadata.csv"
TEMP_ID_FILE="${SCRIPT_DIR}/temp_valid_ids.txt"

# Input files to process (NOTE: these are the *output* of Script 1)
declare -a INPUT_FILES=(
    "${SCRIPT_DIR}/BambuOutput_counts_gene_filtered.txt"
    "${SCRIPT_DIR}/BambuOutput_counts_transcript_filtered.txt"
    "${SCRIPT_DIR}/BambuOutput_CPM_transcript_filtered.txt"
    "${SCRIPT_DIR}/BambuOutput_fullLengthCounts_transcript_filtered.txt"
    "${SCRIPT_DIR}/BambuOutput_uniqueCounts_transcript_filtered.txt"
)
